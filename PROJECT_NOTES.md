...
**ALL agents use `openai/gpt-4.1-mini`** exclusively (no Gemini/Groq — removed Session 13).
All LLM calls route through the Traccia governance proxy (localhost:8001) for real-time monitoring.
...
|93 | **Heartbeat optimized** | DONE | Session 9: `every: "60m"`, `model: "openai/gpt-4.1-nano"`, `lightContext: true`, `activeHours: 08:00-24:00 IST`, `target: "none"`. Saves ~95% on background heartbeat costs. |
|94 | **Compaction optimized** | DONE | Session 9: `model: "openai/gpt-4.1-nano"` for summarization, `maxHistoryShare: 0.4`, `recentTurnsPreserve: 3`, `memoryFlush.enabled: true`. Reduces context bloat in long sessions. |
|95 | Docs updated with cost optimization | DONE | Added "Cost Optimization" section to README, Session 9 entries to PROJECT_NOTES |
|96 | **Shared workspace convention added** | DONE | Session 9: Sub-agents have isolated workspaces. SOUL.md now enforces absolute path `try_out_demos/{project}/` for ALL inter-agent file exchange. |
|97 | **Sequential spawning enforced** | DONE | Session 9: Master-manager was spawning agents in parallel (coder before researcher finished → file not found). SOUL.md now has hard sequential rule: spawn one → WAIT → verify file → spawn next. |
|98 | **Research pipeline with .docx output** | DONE | Session 9: Full research pipeline added: researcher → verify → architect → verify → coder (python-docx) → verify → QA. Coder must generate .docx via Python script execution. Default output is .docx for all research/thesis tasks. |
|99 | **Self-sufficiency rules added** | DONE | Session 9: Sub-agents instructed to install packages, search web, debug errors themselves. No "please provide" — figure it out or work with what you have. |
|100| **ResearchGate 403 workaround** | DONE | Session 9: Researcher agent instructed to skip ResearchGate (blocks bot IPs) and try MDPI, IEEE, Springer, ScienceDirect, Google Scholar instead. |
|101| **Zero-question autonomy enforced** | DONE | Session 10: Master-manager was saying "Let me know if you want me to proceed" between steps (user had to type "yes" twice). Added FORBIDDEN PHRASES list and explicit "ZERO messages between steps" rule. |
|102| **For-loop laziness bug identified** | DONE | Session 10: Coder agent wrote 3 sentences per chapter, stored in dict, looped same text under every heading = fake 15-page doc. Root cause: gpt-4.1-mini 8K output token limit can't generate 15 pages in one script. |
|103| **Chapter-by-chapter multi-chunk strategy** | DONE | Session 10: Instead of one coder writing everything, master-manager now spawns coder ONCE PER CHAPTER. Each writes 500+ words of unique content to `chapters/chapter_N.md`. Final coder compiles all chapters into .docx via python-docx. Bypasses the output token limit completely. |
|104| **Game pipeline added to SOUL.md** | DONE | Session 11: Full game/interactive app pipeline with single-file preference (one .py), programmatic assets, requirements.txt, QA launch test, optional .exe packaging. |
|105| **Website pipeline added to SOUL.md** | DONE | Session 11: Web pipeline with file-manifest verification, CDN-only libraries, picsum.photos placeholders, QA checks every src= and href=. Single-file index.html preference for simple sites. |
|106| **Data analysis pipeline added to SOUL.md** | DONE | Session 11: Code-first data analysis pipeline using yfinance/pandas/matplotlib for market/financial tasks. Avoids LLM financial reasoning (exceeds gpt-4.1-mini capability). Reference: `try_out_demos/market_analysis/step1_fetch_data.py`. |
|107| **Error recovery patterns + quality gates added** | DONE | Session 11: 8 specific error patterns with recovery actions. Quality gates (file existence + size checks) for documents, games, websites, and data analysis. Max 3 retries per agent per step. |
|108| **AGENTS.md fully rewritten** | DONE | Session 11: Fixed all model assignments (was showing Qwen/DeepSeek/Llama — all → gpt-4.1-mini). Added bug #24852 note. Sequential collaboration pattern with WAIT markers. Fixed rule 5: write to absolute path, not /sandbox/. |
|109| **All 6 sub-agent SOUL.md files hardened** | DONE | Session 11: Added content quality rules (anti-for-loop), self-sufficiency rules (install packages, debug errors, never ask user), output path rule (absolute path, not /sandbox/), domain-specific quality checks per agent. |
|110| **BLUEPRINT.md synced with Sessions 9-11** | DONE | Session 11: Added Section 11 (Operational Strategies), updated Section 5 with cost optimization table, added try_out_demos/ to directory tree, added Warnings #6-9. |
|111| **Reference openclaw.json hardened** | DONE | Session 11: Added gateway.tools.allow, expanded acp with enabled/dispatch/backend/allowedAgents, added plugins.entries.acpx (command:node, strictWindowsCmdWrapper:false), added subagents.allowAgents to master-manager. |
|112| **Token mismatch fix strategy — fully automatic** | DONE | Session 11: `scripts/sync-token.sh` detects + fixes all 5 token problems: placeholder in runtime config, auth mismatch, missing token, stale device tokens, broken config. Critical bug fixed: `openclaw config get` reads env vars first (returns placeholder), script now reads JSON file directly via node to get the true runtime value. Direction logic: if runtime=placeholder+env=real → sync runtime←env (NOT env←runtime). `scripts/start.sh` runs sync-token.sh as first step, then background-watches for gateway ready and auto-opens tokenized dashboard URL in browser. |
|113| **`openclaw config get` env-precedence trap discovered** | DONE | Session 11 (live debug): `openclaw config get gateway.auth.token` returns env var value (process env → .env → openclaw.json), NOT the JSON file value. Runtime config had `__OPENCLAW_GATEWAY_TOKEN__` placeholder; CLI returned the placeholder instead of the JSON value, causing sync to go in the wrong direction and overwrite the real token in .env with the placeholder. Fix: `sync-token.sh` now reads `~/.openclaw/openclaw.json` directly via `node -e` to bypass the env-precedence trap. Gateway token regenerated: `0f9760d9...`. |
|114| **Windows Scheduled Task service lock conflict** | DONE | Session 11 (live debug): `openclaw gateway --force` killed process on port 3000 (pid 25000) but found the Windows Scheduled Task lock still held → "lock timeout after 5000ms" → gateway exited. Root cause: `--force` releases the port but not the service lock. Fix added to `start.sh`: (1) `openclaw gateway stop` (releases service lock), (2) `schtasks /End /TN "OpenClaw Gateway"` (Windows Task Scheduler kill), (3) PowerShell force-kill anything on port 3000, (4) 2s sleep for OS to release lock file. Then `--force` succeeds. |
|115| **Dashboard URL watcher timing fixed** | DONE | Session 11 (live debug): Background watcher ran `openclaw dashboard --no-open` while gateway was still starting → browser opened to "site cannot be reached". Fix: watcher now polls `http://127.0.0.1:3000/` and `http://127.0.0.1:3000/__openclaw__/canvas/` and waits for a real HTTP response before generating the URL and opening the browser. |
|116| **Python venv enforcement added to pipelines** | DONE | Session 12: Windows blocks `pip install` to global site-packages (permission error). Both Data Analysis pipeline and General Software pipeline in SOUL.md now ALWAYS create a venv first: `python -m venv venv && source venv/Scripts/activate` (Windows Git Bash path — NOT `venv/bin/activate`). All coder agent task templates include venv creation step before any pip install. |
|117| **Modern JavaScript Framework Pipeline added (Vite)** | DONE | Session 12: Create React App is deprecated (last release 2022, npm install takes 5+ min). New pipeline uses Vite: `npm create vite@latest {project} -- --template react` (~20s scaffold, minimal deps). Pipeline: scaffold coder → implement coder → QA runs `npm run build`. Dev server at `http://localhost:5173`, production at `dist/`. Added to SOUL.md as standalone pipeline section. |
|118| **Tool-maker registration loop fixed (safe HITL pattern)** | DONE | Session 12: Research finding: `openclaw mcp add` does NOT exist in v2026.3.13. MCP servers registered via `openclaw config set plugins.entries.acpx.config.mcpServers.*` but this requires a gateway restart which kills all active sessions — deadlock if attempted mid-session. Safe alternative: tool-maker builds the server, writes `REGISTER.md` (manual instructions) and `scripts/register-{tool-name}.sh` (automated config set commands), then master-manager sends HITL message telling the user to run the script when ready. Added to SOUL.md "Building a Missing Tool Pipeline" section and `agents/tool-maker/SOUL.md`. |
|119| **`openclaw config get` bypass confirmed** | DONE | Session 12: Confirmed root cause of Session 11 token bug. `openclaw config get` reads env vars in order: process env → ./.env → ~/.openclaw/.env → openclaw.json (JSON value last). So `openclaw config get` NEVER reliably returns the JSON file value if any env var overrides it. Any code that needs the actual JSON value must use `node -e "JSON.parse(fs.readFileSync(...))"` directly. This is why sync-token.sh bypasses the CLI and reads the file directly. |
|120| **`start.sh` closing bug: MSYS2 `cmd.exe /c` path conversion** | DONE | Session 12 (live debug): `cmd.exe /c "schtasks /End /TN ..."` in Git Bash (MINGW64) — MSYS2 converts the leading `/c` argument to `C:/` (Windows drive path) BEFORE passing it to cmd.exe. cmd.exe never receives the `/c` flag, starts interactive mode, shows the Windows banner + `C:\...\wow_ai>` prompt, and BLOCKS the bash script waiting for input. Script appears to "close" because cmd.exe captured the terminal. Fix: replaced all `cmd.exe /c` calls with `powershell.exe -Command` equivalents. `Stop-ScheduledTask` for schtasks, `Start-Process` for browser open. |
|121| **`openclaw gateway run --force` (explicit subcommand)** | DONE | Session 12: Changed `openclaw gateway --force` to `openclaw gateway run --force`. The `run` subcommand is the explicit, documented way to run the gateway in foreground mode. The old `openclaw gateway --force` relied on an implicit default behavior that may change in future versions. `openclaw gateway run --help` confirms `--force` is a valid option on the `run` subcommand. |
|122| **Version check added to start.sh** | DONE | Session 12: start.sh now compares `openclaw --version` vs `npm show openclaw version` on every startup and logs a warning if an update is available with the upgrade command. Non-blocking — script continues even if npm check fails. Latest version as of 2026-03-27: `2026.3.24` (vs `2026.3.13` installed). The v2026.3.24 change relevant to us: adds `/v1/models` + `/v1/embeddings` to gateway HTTP API — no breaking changes for our config. Safe to upgrade: `npm install -g openclaw@latest && openclaw doctor --fix`. |
|123| **README.md and Known Issues updated** | DONE | Session 12: Added MSYS2 cmd.exe bug, pip venv fix, and upgrade instructions to Known Issues and Troubleshooting tables. Updated version references to `v2026.3.13+`. Added `sync-token.sh` to project structure listing. |
|124| **Traccia governance integration — proxy architecture** | DONE | Session 12: `wow_traccia_integration.py` was a standalone demo bypassing OpenClaw entirely — useless for real tracing. Solution: OpenAI API proxy (`traccia/proxy.py`) — FastAPI server on `:8001` that intercepts ALL OpenClaw → OpenAI calls. One config change in `~/.openclaw/openclaw.json` routes all 7 agents through proxy. Traccia receives real token counts, costs, latency, and agent names from production traffic. |
|125| **OpenClaw baseUrl changed to Traccia proxy** | DONE | Session 12: `~/.openclaw/openclaw.json` → `models.providers.openai.baseUrl` changed from `https://api.openai.com/v1` to `http://localhost:8001/v1`. Also updated `openclaw/openclaw.json` (reference config) with full `models.providers.openai` section. Rollback: change baseUrl back to `https://api.openai.com/v1`. |
|126| **Traccia proxy streaming + token counting** | DONE | Session 12: OpenClaw uses streaming for long LLM responses. Proxy handles both streaming and non-streaming. For streaming: passes SSE chunks through byte-by-byte, extracts real token counts from the final chunk's `usage` field (requires `stream_options: {include_usage: true}` injected by proxy). For non-streaming: extracts from response `usage` field directly. Real token counts reported to Traccia, not estimates. |
|127| **Traccia policy enforcement in proxy** | DONE | Session 12: Policy engine in `traccia/proxy.py` with 3 policies: (1) hard-block if single call > 30K tokens, (2) soft-warn if session cost > $0.50, (3) hard-block if unknown agent name. Policies emit `guardrail_span` events that appear in Traccia "Detected Patterns" tab. |
|128| **PostgreSQL startup fixed — handles already-running container** | DONE | Session 12: `start.sh` was running `docker compose up -d postgres 2>/dev/null` which silently failed when container was already running or in an unexpected state. Fix: 3-branch logic: (1) if container already running → skip create, just verify health; (2) if container exists but stopped → `docker start wow-ai-postgres`; (3) if container doesn't exist → `docker compose up -d postgres`. Uses `docker exec` for health check (works even if `docker compose exec` path fails). |
|129| **Traccia proxy startup added to start.sh** | DONE | Session 12: `start.sh` step 5b: if `TRACCIA_API_KEY` is set and valid, runs `start-traccia-proxy.sh` before OpenClaw gateway. Proxy must be running BEFORE OpenClaw starts because OpenClaw calls `baseUrl` on startup for model discovery. If proxy fails, gateway falls back to direct OpenAI (graceful degradation). |
|130| **TRACCIA_API_KEY fixed in .env** | DONE | Session 12: Key was accidentally saved in Windows CMD syntax `set "TRACCIA_API_KEY=..."` instead of Unix `.env` format. Fixed to `TRACCIA_API_KEY=tr_dev_...`. Added proper section to `.env.example` with documentation. |
|131| **Traccia dashboard showing "governance.proxy" agent + 0 tokens — fixed** | DONE | Session 13: Root cause: `traccia.init(auto_start_trace=True)` created a background trace on module load, named after the calling module path (`governance.proxy`). This appeared as a spurious agent. Also, `@traccia.observe(as_type="span")` with a plain string return had no token metadata — Traccia showed 0 tokens. Fix: (1) Changed `auto_start_trace=False` so no background trace is created on startup. (2) Changed `as_type="llm"` and added `attributes` dict with `gen_ai.usage.prompt_tokens`, `gen_ai.usage.completion_tokens`, `gen_ai.request.model`, `llm.usage.cost_usd` — Traccia renders these as proper token/cost columns. |
|132| **Removed Gemini/Groq — OpenAI-only** | DONE | Session 13: User had Gemini 503 errors on every `web_search` because OpenClaw used `GEMINI_API_KEY` for search grounding. Fix: removed `GEMINI_API_KEY` and `GROQ_API_KEY` from `~/.openclaw/openclaw.json` plugins acpx_auth env. Also removed `google/gemini-2.5-flash` and `groq/llama-3.3-70b-versatile` from `agents.defaults.model.fallbacks` and `agents.defaults.models`. Platform now uses OpenAI exclusively. web_search is temporarily disabled until `BRAVE_API_KEY` is obtained (free at brave.com/search/api). |
|133| **MCP servers added — Brave Search + Fetch + Postgres** | DONE | Session 13: Added 3 MCP servers to `~/.openclaw/openclaw.json` and reference `openclaw/openclaw.json`: (1) `brave-search` — `@modelcontextprotocol/server-brave-search` — replaces Gemini for `web_search`, needs `BRAVE_API_KEY` (free tier 2000 req/month); (2) `fetch` — `@modelcontextprotocol/server-fetch` — enhanced full-page content extraction, no key needed; (3) `postgres` — `@modelcontextprotocol/server-postgres` — direct DB queries by agents. These are npx-based, auto-installed on first use. |
|134| **Deleted obsolete standalone demo files** | DONE | Session 13: Deleted `traccia_bootstrap.py` (one-time init snippet used during integration setup) and `wow_traccia_integration.py` (standalone LangGraph demo that was superseded by `governance/proxy.py`). Both were no longer needed and created confusion. `try_out_demos/` folder retained. |
|135| **Proxy token policy was blocking master-manager — fixed** | DONE | Session 13: `POLICIES["max_tokens_per_call"]` was 30,000. Master-manager SOUL.md (~34K chars) + conversation history produces ~60K estimated tokens — triggering a hard-block. Error: `400 Policy violation: Agent 'master-manager' estimated 46287 tokens - exceeds hard limit of 30000`. Fix: (1) Raised limit to 500,000 (gpt-4.1-mini supports 1M input; 500K = max ~$0.20/call — blocks runaway loops not normal use). (2) Raised session cost warn threshold from $0.50 to $2.00. (3) Fixed estimation formula from `len(text)//4` to `len(text)//3` (more accurate for prose — was double-counting). |
|136| **SOUL.md was 34,448 chars — OpenClaw truncates at 20,000 — fixed** | DONE | Session 13: OpenClaw silently truncates workspace bootstrap files at 20,000 chars. With 34,448 chars, the last 14,448 chars were dropped — including ALL of: Spawning Rules, Communication Rules, Self-Sufficiency, Quality Assurance, Self-Healing, Constraints. This caused master-manager to ask questions instead of autonomously delegating (autonomy rules were in the cut section). Fix: Rewrote SOUL.md compressing verbose pipeline step-by-step blocks into compact descriptions while preserving all critical rules verbatim. New size: 17,699 chars. All sections now within the 20K limit. Synced to `~/.openclaw/workspace-master-manager/SOUL.md` and `~/.openclaw/agents/master-manager/agent/SOUL.md`. |
|137| **docker-compose.yml had GROQ_API_KEY and GEMINI_API_KEY references — fixed** | DONE | Session 13: docker-compose.yml was passing `GROQ_API_KEY=${GROQ_API_KEY}` and `GEMINI_API_KEY=${GEMINI_API_KEY}` to the openclaw-gateway container. Since we removed those keys from .env, docker-compose printed warnings on every start.sh run. Fixed: replaced with `OPENAI_API_KEY=${OPENAI_API_KEY}` and `BRAVE_API_KEY=${BRAVE_API_KEY:-}` (with default empty value so no warning when key is absent). |
